 ...
 */
/** XMPPHP_XMLStream */
//require_once dirname(__FILE__) . "/XMPPHP_XMPP.php";
...
class XMPPHP_BOSH extends XMPPHP_XMPP {
  ...
  public function connectBosh($server, $wait='1', $session=false) {
  ...
  public function __process($maximum=null) {
    if ($this->http_buffer) {
      $this->__parseBuffer();
    } else {
      $this->__sendBody();
 ...
 return  [ 'rid'=> (string) $this->rid,
    'sid'=> (string) $this->sid,
    'authed' => (boolean) $this->authed,
      'fulljid'=> (string) $this->fulljid];
  }
